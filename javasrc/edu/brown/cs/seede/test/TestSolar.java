...
public class TestSolar implements MintConstants, SesameConstants
{
...
private static final String		MINT_NAME = "SEEDE_TEST_solar";
private static final String		SOURCE_ID = "SEED_22545";
...
private static final String		LAUNCHPOLE_NAME = "solar";
...
@BeforeClass public static void setupBedrock()
{
   ...
   if (!ec1.exists()) {
      ...
      ec2 = new File("/Users/spr/Documents/solar");
    }
   if (!ec1.exists()) {
...
@Test public void testSolar()
{
   ...
   xw.close();
   MintDefaultReply rply = new MintDefaultReply();
   sendSeedeMessage("BEGIN",TESTSOLAR_SID,null,cnts,rply);
   Element status = rply.waitForXml();
   Assert.assertTrue(IvyXml.isElement(status,"RESULT"));

   rply = new MintDefaultReply();
   seede_result = null;
   CommandArgs args = new CommandArgs("EXECID","testsolar","CONTINUOUS",true);
   sendSeedeMessage("EXEC",TESTSOLAR_SID,args,cnts,rply);
   ...
   rply = new MintDefaultReply();
   sendSeedeMessage("REMOVE",TESTSOLAR_SID,null,null,rply);
   sstatus = rply.waitForString();

   rply = new MintDefaultReply();
   sendSeedeMessage("BEGIN",TESTSOLAR_SID1,null,cnts,rply);
   status = rply.waitForXml();
   Assert.assertTrue(IvyXml.isElement(status,"RESULT"));

   rply = new MintDefaultReply();
   seede_result = null;
   args = new CommandArgs("EXECID","testsolar1","CONTINUOUS",true);
   sendSeedeMessage("EXEC",TESTSOLAR_SID1,args,cnts,rply);
   sstatus = rply.waitForString();
   System.err.println("RESULT1 IS " + sstatus);
   ...
   xml = waitForSeedeResult();
   System.err.println("SEED RESULT2 IS " + IvyXml.convertXmlToString(xml));
}
...
private class SeedeHandler implements MintHandler {
   ...
   @Override public void receive(MintMessage msg,MintArguments args) {
      ...
      switch (what) {
         case "EXEC" :
            synchronized (TestSolar.this) {
               seede_result = xml;
               TestSolar.this.notifyAll();
             }
            break;
       }
      msg.replyTo();
